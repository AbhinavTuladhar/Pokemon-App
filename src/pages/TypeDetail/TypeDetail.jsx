...
const TypeDetail = () => {
  ...
  const doubleDamageFromList = extractedInformation?.doubleDamageFrom?.map((type, index) => (
    <TypeCard typeName={type} key={index} />
  ))
  const doubleDamageToList = extractedInformation?.doubleDamageTo?.map((type, index) => (
    <TypeCard typeName={type} key={index} />
  ))
  const halfDamageFromList = extractedInformation?.halfDamageFrom?.map((type, index) => (
    <TypeCard typeName={type} key={index} />
  ))
  const halfDamageToList = extractedInformation?.halfDamageTo?.map((type, index) => (
    <TypeCard typeName={type} key={index} />
  ))
  const noDamageFromList = extractedInformation?.noDamageFrom?.map((type, index) => (
    <TypeCard typeName={type} key={index} />
  ))
  const noDamageToList = extractedInformation?.noDamageTo?.map((type, index) => (
    <TypeCard typeName={type} key={index} />
  ))

  const titleDiv = (
    <motion.h1
      className="flex flex-row justify-center text-4xl font-semibold"
      variants={simpleFadeInVariant}
    >
      {formattedType}&nbsp;
    ...
    <motion.div className="flex flex-col flex-wrap" variants={simpleFadeInVariant}>
      ...
      {doubleDamageToList?.length > 0 && (
        <>
          ...
          <motion.div className="ml-4 flex flex-row flex-wrap gap-2">
            {doubleDamageToList}
          </motion.div>
        </>
      )}
      ...
      <motion.div className="ml-4 flex flex-row flex-wrap gap-2" variants={simpleFadeInVariant}>
        {halfDamageToList}
      </motion.div>
  ...
  return (
    <motion.div exit={{ x: '100%', opacity: 0, transitionDuration: '0.5s' }}>
      <motion.div vairants={simpleFadeInVariant} initial="initial" animate="animate">
        <motion.div variants={simpleFadeInVariant}>
          <motion.div variants={simpleFadeInVariant}>{titleDiv}</motion.div>
          <motion.div variants={simpleFadeInVariant}>
        ...
        <motion.div
          className="mt-4 flex flex-row flex-wrap justify-between"
          variants={simpleFadeInNoDelayVariant}
        >
          <motion.div variants={simpleFadeInNoDelayVariant} className="w-full mdlg:w-1/3">
            ...
            <motion.div variants={simpleFadeInNoDelayVariant}>{defensiveDiv}</motion.div>
          </motion.div>

          <motion.div
            variants={simpleFadeInNoDelayVariant}
            className="mt-4 flex w-full flex-col pl-0 mdlg:mt-0 mdlg:w-2/3 mdlg:pl-16"
          >
        ...
        <motion.div variants={simpleFadeInVariant}>
          {
            <PokemonCardList
              ...
              pokemonUrls={extractedInformation.pokemonList}
            />
          }
        </motion.div>
      </motion.div>
