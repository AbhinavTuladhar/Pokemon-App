...
const TableCell = ({ children, extraClassName }) => {
  return (
    <div className={`table-cell px-2 py-1 align-middle border border-gray-200 ${extraClassName} w-36 whitespace-nowrap lg:whitespace-normal lg:max-w-xs`}>
      {children}
    </div>
  )
}
...
const NatureListing = () => {
  ...
  const tableHeaderNames = [
    'Nature', 'Increases', 'Decreases', 'Likes', 'Hates'
  ]
  const tableHeader = (
  ...
  const tableRows = natureData?.filter(nature => nature !== undefined)?.map((nature, rowIndex) => {
    const {
      decreasedStat,
      hatesFlavour,
      increasedStat,
      ...
      <main className='block md:flex md:flex-col md:items-center'>
        {isFullyLoadedNatureData && natureData?.length > 0 ? (
          <div className='overflow-x-auto overflow-y-hidden'>
            <div className='border-b border-slate-400 table'>
              {tableData}
            </div>
          </div>
        ) : (
